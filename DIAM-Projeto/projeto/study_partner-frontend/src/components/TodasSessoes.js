...
import { DELETE_UC_URL, deleteFactory, GET_UCS_URL } from '../Constants';
        ...
        {filteredUcs.map((uc) => (
          <UcCard
            ...
            onDelete={
              async () => {
                try {
                  await deleteFactory(DELETE_UC_URL, uc.code)
                  setUcs(prevUcs => prevUcs.filter(u => u.code !== uc.code));
                } catch (err) {
                  console.error(err);
                  alert("Erro: Não foi possível apagar a uc")
                }
              }
            }
          />
